...
description: 切换、记住并使用当前 Codex 环境的项目指令文件来源，让后续任务持续使用目标目录下的 AGENTS.md 和相关指令文件，直到切换或清除。适用于用户要 set、use、show、switch、temp-use 或 clear 指令来源目录，或当前环境已有保存来源、需要按该来源继续处理后续任务时。
---
...
Use this skill to switch the project-level instruction-file source for the current Codex environment. Once a source is saved with `set` or `switch`, later tasks in this environment should use that source until it is replaced or cleared. The source must be a directory containing `AGENTS.md`; every file under that directory is a potential instruction file, and the target `AGENTS.md` decides which files to read and how to use them.

This skill does not override system, developer, or current user messages. It only changes the environment-level project instruction source you consult while doing the user's work. When cleared, the environment falls back to its normal instruction files for the current working context.

## Commands

- `set /path/to/instructions`: save this directory as the environment's instruction-file source.
- `use`: use the saved environment instruction-file source.
- `show`: show the saved environment instruction-file source.
- `switch /path/to/instructions`: replace the saved environment instruction-file source.
- `temp /path/to/instructions`: use this source for the current request only, without saving it.
- `clear`: clear the saved environment instruction-file source and fall back to the current context's normal instruction files.

Examples:

- `[$instruction-source-switcher] set /Users/jp/Documents/jp_agents`: 设置并记住这个目录作为指令文件来源。
- `[$instruction-source-switcher] use`: 使用当前环境已记住的指令文件来源，先读目标 `AGENTS.md`，再按其中说明继续工作。
...
- `[$instruction-source-switcher] clear`: 清除当前环境已保存的指令文件来源，恢复使用当前上下文的默认指令文件。

## Workflow

1. Parse the user's command.
   - For `set` and `switch`, run `scripts/instruction_profile.py set <path>` and treat the validated source as the environment's persisted instruction-file source for later tasks.
   ...
   - For `clear`, run `scripts/instruction_profile.py clear`, report that the environment has fallen back to the current context's normal instruction files, and stop.
...
The saved environment source is stored in `~/.codex/state/agent-instruction-files.json`.
