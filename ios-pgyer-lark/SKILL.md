...
飞书通知会显示当前 Git 分支，并默认包含最近 3 条 Git 提交标题；只发送标题，不发送作者、commit hash 或 diff，并会对明显的密钥、token、webhook 和本机路径做脱敏。
...
- 飞书通知会在「版本号」下一行显示「当前分支」；无法获取时显示「未获取到 git 分支」。
- 如果 `.app` 内存在有效的 `${App名字}BuildInfo.plist`，且其中 `BuildTime` 是非空字符串，飞书通知会在「当前分支」下一行补充「构建时间」。
